...
const INPUT_CLASS = "w-full mt-1 px-3 py-2 rounded-lg bg-zinc-900 border border-zinc-700 text-white text-xs outline-none focus:border-primary transition-colors";
...
export default function AnnouncementModal() {
  ...
  const save = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setData(next);
      setIsEditing(false);
      if (next.is_active && localStorage.getItem(STORAGE_KEY) !== next.version) {
        setIsOpen(true);
      ...
      toast({
        title: "保存失败",
        description: error.message || "更新出错，请检查管理员权限",
        variant: "destructive",
      });
    } finally {
      setSaving(false);
  ...
  if (!isOpen && !isEditing) {
    ...
    return (
      <button
        ...
        onClick={startEdit}
        title="管理员编辑公告"
        className="fixed bottom-6 right-6 z-40 flex items-center gap-1.5 px-3.5 py-2 rounded-full bg-zinc-950/90 hover:bg-zinc-900 text-amber-400 hover:text-amber-300 border border-amber-500/40 shadow-lg shadow-amber-500/20 text-xs font-medium backdrop-blur-md transition-all hover:scale-105"
      >
    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
        <motion.div
          ...
          exit={{ opacity: 0 }}
          className="relative w-full max-w-md rounded-2xl bg-zinc-950/95 border border-primary/40 shadow-2xl shadow-primary/20 p-6 overflow-hidden text-left"
        >
          ...
          <div className="absolute top-4 right-4 flex items-center gap-2">
            {isAdmin && !isEditing && (
              <button
                ...
                onClick={startEdit}
                title="管理员编辑"
                className="p-1.5 rounded-lg bg-primary/10 hover:bg-primary/20 text-primary transition-colors text-xs flex items-center gap-1 border border-primary/30"
              >
          ...
          {isEditing ? (
            ...
            <div className="space-y-4 pt-1">
              ...
              <h3 className="text-lg sm:text-xl font-bold text-white tracking-wide">{data.title}</h3>
              ...
              <div className="space-y-2 pt-2">
                <button
                  ...
                  className="w-full py-3 rounded-xl bg-gradient-to-r from-primary via-amber-400 to-yellow-500 text-black font-bold flex items-center justify-center gap-2 shadow-lg hover:opacity-95 transition-opacity"
                >
                ...
                <button
                  type="button"
                  onClick={close}
                  className="w-full py-2.5 rounded-xl bg-zinc-900/80 hover:bg-zinc-800 text-zinc-400 text-xs border border-zinc-800 transition-colors"
                >
                  我知道了 · 进入网站
                </button>
              </div>
