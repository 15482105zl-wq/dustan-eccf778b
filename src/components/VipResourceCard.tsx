...
const VipResourceCard = ({ icon: Icon, title, subtitle, url, onClick, delay = 0, badge = false }: Props) => {
  ...
  return (
    <motion.div
      ...
      whileTap={{ scale: 0.98 }}
      onClick={handleClick}
      className="bg-glass/60 backdrop-blur-xl rounded-xl p-3 cursor-pointer relative overflow-hidden group transition-colors duration-300 min-h-[108px] flex flex-col items-center justify-center text-center border border-glass-border/40 hover:border-primary/40 animate-breathe-glow"
    >
      {badge && (
